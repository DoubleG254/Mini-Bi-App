        ...
        for r in results:
            ...
            try:
                ...
                # If it's a Chart (has 'x_col' and 'y_col'), handle it
                    ...
                    chart_type = chart_data.get("chart_type") or "scatter"  # Default to scatter if missing
                    
                    # Ensure columns exist
                    if chart_data["x_col"] not in df.columns or chart_data["y_col"] not in df.columns:
                        print(f"Warning: Columns {chart_data['x_col']} or {chart_data['y_col']} not found. Skipping.")
                        continue

                    clean_df = df.dropna(subset=[chart_data["x_col"], chart_data["y_col"]])
                    
                    if chart_type == "line" or chart_type == "bar":
                        if chart_type == "line":
                            data = clean_df.groupby(chart_data["x_col"])[chart_data["y_col"]].mean().sort_index().to_dict()
                        else:
                            data = clean_df.groupby(chart_data["x_col"])[chart_data["y_col"]].mean().sort_values(ascending=False).to_dict() 
                        ...
                        corr = clean_df[chart_data["x_col"]].corr(clean_df[chart_data["y_col"]])
                        # Convert dataframe to list of dicts for scatter points
                        # Convert numpy types to native Python types
                        points_data = clean_df.apply(
                            lambda row: {
                                "x": float(row[chart_data['x_col']]) if not pd.isna(row[chart_data['x_col']]) else None,
                                "y": float(row[chart_data['y_col']]) if not pd.isna(row[chart_data['y_col']]) else None
                            },
                            axis=1
                        ).tolist()
                        ...
                        chart_config = {
                            ...
                            "correlation_coefficient": float(corr)  # Convert numpy float to Python float
                        }
                        charts.append(chart_config)
